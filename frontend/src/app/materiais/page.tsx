...
export default function MateriaisListPage() {
  ...
  const load = useCallback(() => {
    ...
    setLoading(true);
    setErr('');
    fetchPaginated<Row>('/materials', page, PAGE_SIZE)
      .then((r) =>
        setData({
          items: r.items,
          total: r.total,
  ...
  async function confirmRemove() {
    if (!removeId) return;
    try {
      await api(`/materials/${removeId}`, { method: 'DELETE' });
      setRemoveId(null);
      load();
    } catch (e) {
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Materiais recicláveis</h1>
            ...
            {loading ? (
              ...
              data.items.map((r) => (
                <tr key={r.id} className="border-t border-slate-100">
                  ...
                  <td className="px-4 py-3">{r.materialType.name}</td>
                  <td className="px-4 py-3">{r.unit.code}</td>
                  <td className="px-4 py-3">{r.active ? 'Sim' : 'Não'}</td>
                  <td className="px-4 py-3">
                    <ListRowActions
                      viewHref={`/materiais/${r.id}`}
                      editHref={`/materiais/${r.id}/editar`}
                      showEdit={canMutate}
      ...
      <ConfirmModal
        ...
        message="Deseja desativar este material?"
