...
export default function RamosAtividadePage() {
  ...
  const load = useCallback(() => {
    ...
    setLoading(true);
    setErr('');
    fetchPaginated<Row>('/activity-branches', page, PAGE_SIZE)
      .then((r) =>
        setData({
          items: r.items,
          total: r.total,
  ...
  async function confirmRemove() {
    ...
    setErr('');
    try {
      await api(`/activity-branches/${removeId}`, { method: 'DELETE' });
      setRemoveId(null);
      load();
    } catch (e) {
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Ramos de atividade</h1>
          <p className="text-slate-600 text-sm mt-1">
            Cada ramo é vinculado a depósito ou destino final e usado no cadastro de estabelecimentos.
          </p>
        </div>
        {canMutate && (
          <Link
            href="/configuracoes/ramos-atividade/novo"
              ...
              data.items.map((r) => (
                <tr key={r.id} className="border-t border-slate-100">
                  ...
                  <td className="px-4 py-3">
                    <ListRowActions
                      viewHref={`/configuracoes/ramos-atividade/${r.id}`}
                      editHref={`/configuracoes/ramos-atividade/${r.id}/editar`}
                      showEdit={canMutate}
      ...
      <ConfirmModal
        open={!!removeId}
        title="Excluir ramo"
        message="Só é possível excluir se nenhum estabelecimento usar este ramo."
